...
class ShoppingItem {
  ...
  static ShoppingItem fromInput(String input) {
    ...
    if (prefixMatch != null) {
      ...
      final suffixMatch = RegExp(r'^(.+?)\s+(\d+\s*(?:x|X|×|g|kg|ml|l|L|St|Stk|Pkg|Pckg)\.?)$').firstMatch(trimmed);
      if (suffixMatch != null) {
  ...
  static String _guessEmoji(String name) {
    ...
    if (lower.contains('milch') || lower.contains('joghurt') || lower.contains('kaese') || lower.contains('butter') || lower.contains('sahne') || lower.contains('quark')) return '\u{1F95B}';
    if (lower.contains('brot') || lower.contains('broetchen') || lower.contains('toast')) return '\u{1F35E}';
    if (lower.contains('obst') || lower.contains('apfel') || lower.contains('banane') || lower.contains('erdbeere') || lower.contains('orange')) return '\u{1F34E}';
    if (lower.contains('gemuese') || lower.contains('karotte') || lower.contains('brokkoli') || lower.contains('tomate') || lower.contains('salat') || lower.contains('gurke')) return '\u{1F966}';
    if (lower.contains('fleisch') || lower.contains('wurst') || lower.contains('schinken') || lower.contains('hack')) return '\u{1F356}';
    if (lower.contains('nudel') || lower.contains('reis') || lower.contains('mehl') || lower.contains('pasta')) return '\u{1F35D}';
    if (lower.contains('wasser') || lower.contains('saft') || lower.contains('cola') || lower.contains('limo')) return '\u{1F4A7}';
    if (lower.contains('windel') || lower.contains('feuchttuch') || lower.contains('creme')) return '\u{1F476}';
    if (lower.contains('wasch') || lower.contains('spuel') || lower.contains('seife') || lower.contains('shampoo')) return '\u{1F9F4}';
    if (lower.contains('toiletten') || lower.contains('kuechen') || lower.contains('papier')) return '\u{1F9FB}';
    return '\u{1F6D2}';
  }
}
...
class ShoppingListService {
  ...
  Future<void> load() async {
    ...
    _done.removeWhere((item) => item.doneAt != null && item.doneAt!.isBefore(cutoff));
  }
  ...
  Future<void> addItemsFromRecipe(List<String> ingredients) async {
    for (final ing in ingredients) {
      _active.insert(0, ShoppingItem.fromInput(ing));
    }
    await _persist();
  }
  ...
  Future<void> _persist() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString(_activeKey, jsonEncode(_active.map((i) => i.toJson()).toList()));
    await prefs.setString(_doneKey, jsonEncode(_done.map((i) => i.toJson()).toList()));
  ...
  List<ShoppingItem> _loadList(SharedPreferences prefs, String key) {
    ...
    try {
      return (jsonDecode(raw) as List).map((e) => ShoppingItem.fromJson(e)).toList();
    } catch (_) {
      return [];
    }
  }
